...
class DuckBill09
  ...
  def myinit
    ...
    @chase_lim = [300,600,800,1000]
    @tangle = radar_heading
    @duckdbg = false
    @mytrack = Tracking09.new(battlefield_width,battlefield_height,@outerlimits)
    @mycontrol = Controls09.new
  ...
  def tick events
    ...
    #runmode 0 circles around the target
      ...
      if @mytrack.range > @chase_lim[3]
        @runmode = "runto"
        mydir = (@mytrack.angle + @dir * 30)%360
      elsif @mytrack.range > @chase_lim[2]
        mydir = (@mytrack.angle + @dir * 70)%360
      elsif @mytrack.range < @chase_lim[0]
        @runmode = "runfrom"
        mydir = (@mytrack.angle + 180 + @dir * 30)%360
      elsif @mytrack.range < @chase_lim[1]
        mydir = (@mytrack.angle + @dir * 110)%360
      else
      ...
      if (time+@time_sync) % 20 < 9
        stop
      else
        accelerate 1
      end
      if @hit_filter > 6 and @sinceevade > 40
        @sinceevade = 0
        @dir *= -1
        @time_sync = 25 - time%20 
      end
    elsif @runmode == "runfrom"
    #run away
      mydir = (@mytrack.angle + 180 + @dir * 45)%360
      if walls.sort.first < 100 and @mytrack.range < @chase_lim[1]
        @runmode = "runstreight"
        @since_evade = 0
      elsif @mytrack.range > @chase_lim[1]
        @runmode = "rotate"
    ...
    #run towards
      mydir = (@mytrack.angle + @dir * 45)%360
      if @mytrack.range < @chase_lim[1]
        @runmode = "runstreight"
        @since_evade = 0
      elsif @mytrack.range < @chase_lim[2]
        @runmode = "rotate"
    ...
    #just strieght
      ...
      @runmode = "rotate" if walls.sort.first > 300
      accelerate 1
    elsif @runmode == "dance"
    #dance baby dance!
  ...
  def dance
    aimrad(radar_heading + 10)
    ang = Math.atan2(y-battlefield_height/2,battlefield_width/2 - x).to_deg
    aimtank(ang%360)
    aimgun(gun_heading + 3)
    accelerate 1
    fire 0.1
